...
def print_aspects(beta_file, vocab_file, nwords = 25):
    # get the vocabulary
    vocab0file = open(vocab_file, 'r')
    vocab0 = vocab0file.readlines()
    vocab0 = list(map(lambda x: x.strip(), vocab0))
    ...
    topic_no = 0
    aspectsfile = open(beta_file, 'r')
    for aspect in open(beta_file, 'r'):
        aspect = list(map(float, aspect.strip().split()))
        if len(aspect) != wordSize :
            continue
        
        indices.sort(reverse=True, key = lambda x: aspect[x])
        print('Aspect ' + str(topic_no))
